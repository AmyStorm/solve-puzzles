...
class Texteditor:
    ...
    def copy(self):
        self.copied_value = self.current_document.global_text[self.current_document.selected_left: self.current_document.selected_right]
    ...
    def create(self, param):
        ...
        if exist_flag is False:
            self.document.append(Document(param))

    def switch(self, param):
        for e in self.document:
            if e.name == param:
                self.current_document = e
                return
    ...
    def command(self, command_name: str, *params: str):
        ...
        if len(self.document) == 0:
            now = datetime.now()
            self.document.append(Document(str(now)))
            self.switch(str(now))
        if command_name == MOVE:
            ...
            self.create(params[0])
        elif command_name == SWITCH:
            self.switch(params[0])
...
if __name__ == '__main__':
    editor = Texteditor()
    editor.command('CREATE', 'document1')
    editor.command('CREATE', 'document2')
    editor.command('CREATE', 'document1')
    editor.command('SWITCH', 'document1')
    editor.command('APPEND', 'Hello, world!')
    editor.command('SELECT', '7', '12')
    editor.command('COPY')
    editor.command('SWITCH', 'document2')
